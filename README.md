...
- **Mathematics for physics:** a 45-chapter path from algebra, trigonometry and calculus through linear algebra, differential equations, fields, waves, probability, quantum notation and spacetime geometry. Nine intermediate synthesis chapters develop trajectories, the two-body problem, perturbation theory, coordinate field operators, vibrating strings, transport PDEs, electrostatic boundaries, random-walk diffusion and least squares. They appear after their prerequisite tools and before the three final investigations. Worked connections throughout the chapters develop shared ideas including nullspaces, projection, local approximation and conservation.
...
Reader-facing lesson structure is authored through the generic `content` model described in [Curriculum authoring](docs/CURRICULUM_AUTHORING.md). It supports reusable prose, callouts, lists, steps, diagrams, checkpoints, examples, sidebars, labs and takeaways without requiring every lesson to use the same sequence. Legacy topics remain supported while curricula migrate.
