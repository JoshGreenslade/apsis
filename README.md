...
- **Mathematics for physics:** a 36-chapter path from algebra, trigonometry and calculus through linear algebra, differential equations, fields, waves, probability, quantum notation and spacetime geometry. Five recurring threads—local approximation, superposition, invariance, local-to-global reasoning and representation—are made explicit in the course overview so the mathematics is revisited across contexts rather than treated as disconnected techniques.
