...
export const unit07CoordinateAgents: Lesson[] = [
  {
    ...
    sections: [
      ...
      ["Price the coordination", "Every extra agent adds setup, context transfer, conflict resolution, duplicated work and review. Estimate the join before launching the split. If the answer from one worker cannot be evaluated without replaying its whole reasoning, the apparent parallelism may be an expensive illusion."],
    ],
    checkpoints: [
    ...
    checks: [
      ["What makes work separable?", ["Distinct inputs, outputs and a join", "Different agent names", "More tokens"], 0, "Parallelism depends on low coupling and a planned join."],
      ["What is a coordination cost?", ["Context transfer and conflict resolution", "A shorter task", "A stronger assertion"], 0, "Extra agents create work as well as capacity."],
      ["Who should own synthesis?", ["An explicit final owner", "Nobody", "Every worker at once"], 0, "A join needs accountability."],
    ],
  ...
  },
  {
    ...
    sections: [
      ["Give enough context, not the whole world", "State the objective, relevant revision, files or tools, constraints and definition of done. Include pointers to evidence rather than flooding the worker with the entire conversation. A worker that lacks context guesses; one that receives everything spends its budget rediscovering the question."],
    ...
    checks: [
      ["What belongs in a sub-agent brief?", ["Question, scope, evidence and boundary", "Only a topic", "The entire transcript"], 0, "A worker needs enough context to answer a bounded question."],
      ["What should a return contract include?", ["Evidence and uncertainty", "Only a confidence score", "An unrelated patch"], 0, "The caller must be able to inspect the result."],
      ["Why prohibit edits for a mapping worker?", ["To keep ownership and side effects clear", "To reduce all useful work", "To hide its findings"], 0, "Read-only investigation is easier to join safely."],
    ],
  ...
  },
  {
    ...
    intro:
      "A relay team does not pass a baton by shouting the whole race history. It passes a small, well-defined object at a known point. Multi-agent work needs the same discipline: who owns each artifact, which revision it describes, what state it is in and what event permits the next handoff.",
    sections: [
    ...
    checks: [
      ["What should a shared task record contain?", ["Owner, status, revision and evidence", "Only a title", "Private guesses"], 0, "Coordination needs visible state."],
      ["How should conflicts be resolved?", ["By evidence and accountable ownership", "By the longest report", "By hiding both"], 0, "Scope and source matter more than volume."],
      ["Why keep raw findings immutable?", ["To preserve provenance", "To prevent review", "To increase duplication"], 0, "Immutable evidence supports later inspection."],
    ],
  ...
  },
  {
    ...
    sections: [
      ...
      ["Protect the join", "Define the artifact that crosses each boundary, the evidence required and the owner of the final decision. An independent reviewer should not inherit the implementer's conclusion as an instruction; it should receive the task contract, proposed change and relevant evidence, then form its own judgement."],
    ],
    checkpoints: [
    ...
    checks: [
      ["When is fan-out/fan-in useful?", ["Independent questions with a common join", "Tightly coupled edits", "Unbounded exploration"], 0, "The pattern depends on separable work and comparable returns."],
      ["What makes a reviewer independent?", ["Fresh scope and authority to challenge", "The same hidden transcript", "A different avatar"], 0, "Independence concerns context and incentives."],
      ["What should cross a stage boundary?", ["A defined artifact and evidence", "Every prior thought", "An unlabelled guess"], 0, "Stable handoffs keep pipelines inspectable."],
    ],
  ...
  },
  {
    ...
    checks: [
      ["When should one agent usually win?", ["Tightly coupled, small work", "Many independent investigations", "A need for adversarial review"], 0, "Coherence can outweigh parallelism."],
      ["What should trigger escalation?", ["An observed bottleneck or risk", "Architecture fashion", "A larger prompt"], 0, "Add capacity for a reason."],
      ["When should a worker be removed?", ["When it no longer changes a decision", "Never", "When it finishes one sentence"], 0, "Coordination has an ongoing cost."],
    ],
    takeaway: "Start with one coherent owner; add agents only when a specific, measurable bottleneck justifies the join.",
    nextConnection: "With coordination choices understood, the course turns to evaluation: how to know whether an agentic system is actually improving outcomes.",
    source: ["tools", "engines"],
    practical: {
      title: "Lab 7 · Compare one agent with many",
      minutes: 40,
      brief:
