...
export const unit06AddExternalCapabilities: Lesson[] = [
  {
    ...
    intro:
      "A workshop tool is easier to trust when its handle, range and result are obvious. 'Run anything' is a box of unlabelled machinery: powerful, but hard to reason about and easy to misuse. A good agent tool makes one useful action legible, constrains what can happen and returns the evidence needed for the next decision.",
    sections: [
      ...
      ["Treat failures as observations", "Return actionable errors: what was attempted, what was rejected, whether a retry is safe and what the caller can do next. Validate inputs before side effects, make repeated requests safe where possible and avoid returning secrets or unbounded output merely because the underlying system contains them."],
    ],
    checkpoints: [
  ...
  },
  {
    ...
    intro:
      "A plug standard lets a lamp connect to a wall without telling the lamp what room it is in or who may switch it on. MCP plays a similar role for agent-accessible capabilities: it standardises discovery and invocation between a client and a server. The protocol can make connections easier to compose; it does not make an unsafe tool safe or an untrusted server trustworthy.",
    sections: [
      ["Separate protocol from policy", "MCP can describe tools, resources and prompts and carry calls between a client and server. Your system still needs to decide which server is trusted, which identity is used, what data may cross the boundary, how calls are logged and which operations require approval. A protocol describes the road; policy decides who may drive where."],
  ...
  },
  {
    ...
    intro:
      "A recipe is useful because it captures more than ingredients. It says what the dish is for, what substitutions are safe and how to tell when it is done. A skill does the same for an agent: it packages a recurring workflow, the decisions inside it, the evidence it should produce and the boundaries it must respect.",
    sections: [
      ...
      ["Keep skills scoped and testable", "A skill should say when it applies and when it does not. Prefer a short decision tree over a ritual list, and link to the underlying checks rather than claiming that following the prose proves success. Review skills as the repository and tools change, just as you would review code."],
    ],
    checkpoints: [
    ...
    practical: {
      title: "Lab 6 · Design one trustworthy capability",
      minutes: 30,
      brief:
