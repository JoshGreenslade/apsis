...
const chapter: Chapter = {
  theoreticalMinimum: { coreIdea: "A function is a rule with a declared domain, not merely an equation with a symbol on each side. Inversion is valid only when the original mapping is one-to-one on the relevant domain, so a graph and its restrictions are part of the mathematics rather than decoration.", widerConnection: "This discipline of declaring inputs and outputs underlies coordinate changes, state evolution, and every later physical model. Whenever a formula is solved backwards, transformed, or used to infer a hidden quantity, the course will return to the question of which inputs are actually distinguishable." },
  "id": "functions",
  "intuition": {
